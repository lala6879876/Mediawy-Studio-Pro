...
# --- 1. الاستدعاء الحديث والمستقر (حل ModuleNotFoundError) ---
import moviepy.video.VideoClip as vclip
import moviepy.video.compositing.concatenate as vconcat
import moviepy.video.compositing.CompositeVideoClip as vcomp
import moviepy.audio.io.AudioFileClip as aclip
import moviepy.audio.AudioClip as acomp
from moviepy.config import configure

# إعداد ImageMagick للسيرفر (Linux)
try:
    if os.name == 'posix':
        configure(IMAGEMAGICK_BINARY="convert")
    else:
        # لو شغال ويندوز (تأكد من المسار على جهازك)
...
def create_static_layer(size, logo_path, marquee_text):
    ...
    return vclip.ImageClip(np.array(img))
...
def create_text_clip(size, text, start_t, dur):
    ...
    return vclip.ImageClip(np.array(img)).with_start(start_t).with_duration(dur).with_position('center')

# --- 4. واجهة المستخدم ---
st.set_page_config(page_title="Mediawy Pro V21", layout="wide")
st.title("🎬 Mediawy Studio V21 - Final Stable")
...
if st.button("إطلاق خط الإنتاج 🚀", use_container_width=True):
    if not ai_text or not logo_file:
        ...
        try:
            ...
            voice_clip = aclip.AudioFileClip(audio_p)
            ...
            for i, sentence in enumerate(sentences):
                ...
                c = vclip.ImageClip(p).with_duration(dur_per_clip).resized(height=h)
                img_clips.append(c)
                sub_clips.append(create_text_clip((w, h), sentence, i*dur_per_clip, dur_per_clip))

            video_track = vconcat.concatenate_videoclips(img_clips, method="compose")
            ...
            final_vid = vcomp.CompositeVideoClip([video_track, static_layer] + sub_clips, size=(w, h)).with_audio(voice_clip)
            
            out_p = os.path.join(VIDEOS_DIR, "Final_Cloud_V21.mp4")
