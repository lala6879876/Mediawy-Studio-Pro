...
# --- 3. دوال الرسم الاحترافي ---
def process_static_layer(size, logo_path, marquee_text):
    img = Image.new("RGBA", size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    try:
        font = ImageFont.truetype("arial.ttf", size[1] // 25)
    except:
...
def create_text_clip(size, text, start_t, duration):
    img = Image.new("RGBA", size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    try:
...
# --- 4. الواجهة ---
st.set_page_config(page_title="Mediawy Pro V18", layout="wide")
st.title("🎬 Mediawy Studio V18 - Stable")
...
if st.button("إطلاق خط الإنتاج 🚀", use_container_width=True):
    if not ai_text or not logo_file:
        ...
        try:
            ...
            h = 1080
            w = int(h*9/16) if "9:16" in dim else int(h*16/9)
            ...
            st.video(out_p)
            st.success("🔥 المكنة طلعت قماش أونلاين!")
            
        except Exception as e:
            st.error(f"⚠️ حدث خطأ أثناء الإنتاج: {str(e)}")
