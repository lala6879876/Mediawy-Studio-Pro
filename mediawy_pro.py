...
# --- محرك الزووم والتحريك (توافق 100% مع MoviePy 2.x) ---
def apply_pro_zoom(clip, index):
    dur = clip.duration
    # تطبيق الزووم السينمائي (Ken Burns)
    if index % 2 == 0:
...
st.set_page_config(page_title="Mediawy V71", layout="wide")
st.markdown("<h1 style='text-align:center; color:#FF0000;'>🎬 Mediawy Studio <span style='color:#00E5FF;'>V71 Final Success</span></h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    ai_text = st.text_area("✍️ النص:", value="الإرادة القوية تفتح أبواب المستحيل.")
...
if st.button("🚀 إطلاق رندر الإنجاز المصلح", use_container_width=True):
    try:
        status = st.info("⏳ جاري المونتاج بنظام V71 الآمن...")
        ...
        for i, sentence in enumerate(sentences):
            ...
            if img_mode == "أوتوماتيك (سياقي)":
                query = sentence.split()[0] if sentence.split() else "success"
        ...
        # [5] النقلات الناعمة (استخدام method=compose بدلاً من crossfade)
        ...
        out_p = os.path.join(VIDEOS_DIR, "Mediawy_V71_Final.mp4")
