...
st.set_page_config(page_title="Mediawy Mega V36", layout="wide")
st.markdown("<h1 style='text-align:center; color:#FF0000;'>🎬 Mediawy Studio <span style='color:#00E5FF;'>V36 Ultimate</span></h1>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if audio_source == "ElevenLabs":
        ...
        el_text = st.text_area("النص لـ ElevenLabs:")
    ...
    duck_vol = st.slider("مستوى الموسيقى (Ducking):", 0.05, 0.40, 0.10)
    st.divider()
    ...
    show_banner = st.checkbox("تفعيل البنر السفلي", value=True)
...
if st.button("🚀 إطلاق خط الإنتاج الشامل", use_container_width=True):
    if not (ai_text or user_audio) or not logo_file:
        ...
        try:
            status = st.info("⏳ جاري بناء الفيديو بكافة الإضافات...")
            ...
            out_p = os.path.join(VIDEOS_DIR, "Final_Beast.mp4")
            final_vid.write_videofile(out_p, fps=24, codec="libx264")
            
            st.video(out_p)
            st.success("🔥 مبروك! المكنة طلعت قماش كامل الرفاهية!")
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.write("**العنوان والوصف:**")
                st.code(f"سر {sentences[0][:30]}...\nشاهد للنهاية لمعرفة التفاصيل!")
            with c2:
                st.write("**الكلمات والهاشتاجات:**")
