...
# --- 3. محركات الرسم (ثبات الهوية + نصوص Clipchamp) ---
def create_static_layer(size, logo_path, banner_text, show_banner):
    ...
    # 8- بنر سفلي اختياري بنص متحرك (محاكى)
...
def create_word_by_word_clip(size, text, start_t, dur):
    # 7- تأثير الكلمة بكلمة (تمركز ناعم)
    img = Image.new("RGBA", size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
...
st.set_page_config(page_title="Mediawy Mega V37", layout="wide")
st.markdown("<h1 style='text-align:center; color:#FF0000;'>🎬 Mediawy Studio <span style='color:#00E5FF;'>V37 Master</span></h1>", unsafe_allow_html=True)

with st.sidebar:
    st.header("⚙️ مركز التحكم الشامل")
    
    # 1 & 2: المونتاج والأبعاد
    st.subheader("📺 1. ستايل الفيديو")
    dim = st.selectbox("📏 الأبعاد (2):", ["9:16 (Shorts)", "16:9 (YouTube)"])
    edit_style = st.selectbox("🎭 النمط الفني (1):", ["سينمائي 🎬", "درامي 🎭", "وثائقي 📜"])
    st.divider() # 11- فواصل

    # 3: الصوت و ElevenLabs (3 مربعات)
    st.subheader("🎙️ 2. هندسة الصوت (3)")
    ...
    if "ElevenLabs" in audio_source:
        ...
        st.info("3. اكتب النص في المربع أدناه")
    ...
    # 4: الصور (أوتوماتيك أو رفع)
    st.subheader("🖼️ 4. محرك الصور (4)")
    img_mode = st.radio("الجلب:", ["أوتوماتيك (AI السياق)", "يدوي (رفع حتى 500 صورة)"])
    user_imgs = st.file_uploader("ارفع صورك هنا", accept_multiple_files=True)
    st.divider()

    # 8 & 9: الهوية والبنر
    st.subheader("🚩 5. الهوية والبنر (8-9)")
    show_banner = st.toggle("تفعيل البنر السفلي", value=True)
...
if st.button("🚀 إطلاق خط الإنتاج الشامل", use_container_width=True):
    if not (ai_text or user_audio) or not logo_file:
        st.error("⚠️ يرجى إكمال البيانات (النص + اللوجو)!")
    else:
        try:
            status = st.info("⏳ جاري تحليل السياق وبناء الفيديو...")
            ...
            for i, sentence in enumerate(sentences):
                ...
                # 1 & 5: الزووم والنقلات الناعمة
                c = ImageClip(p).with_duration(dur_per_clip).resized(height=h)
                z_factor = 1.25 if i % 2 == 0 else 0.85 # زووم إن وزووم أوت
                ...
                elif "سينمائي" in edit_style:
                    c = c.with_effects([lambda cl: cl.image_transform(lambda im: (im * 1.1).astype('uint8'))])
            ...
            out_p = os.path.join(VIDEOS_DIR, "Mediawy_Ultimate.mp4")
            final_vid.write_videofile(out_p, fps=24, codec="libx264")
            
            st.video(out_p)
            st.success("🔥 المكنة طلعت قماش كامل الرفاهية أونلاين!")
            ...
            with c1:
                ...
                st.code(f"أسرار {sentences[0][:30]}... | مونتاج {edit_style}")
            with c2:
                st.write("**الكلمات المفتاحية والهاشتاجات:**")
